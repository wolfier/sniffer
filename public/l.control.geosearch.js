...
L.Control.GeoSearch = L.Control.extend({
    ...
    onAdd: function (map) {
        ...
        if (!topCenter) {
            ...
            tc.className += 'leaflet-top leaflet-center extra';
            $controlContainer.appendChild(tc);
            map._controlCorners.topcenter = tc;
        }

        this._map = map;
        this._container = L.DomUtil.create('div', 'leaflet-control-geosearch stretch');
    ...
    _showLocation: function (location) {
        if (this.options.showMarker == true) {
            if (typeof this._positionMarker === 'undefined'){
                this._positionMarker = L.circle([location.Y, location.X], 1600).addTo(this._map);
            }
            else{
